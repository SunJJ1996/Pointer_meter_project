...
class craft_base_dataset(data.Dataset):
    def __init__(self, target_size=300, viz=False, debug=False):
...
class ICDAR2013(craft_base_dataset):
    def __init__(self, net, icdar2013_folder, target_size=768, viz=False, debug=False):
        ...
        self.net = net
        self.net.eval()
        self.img_folder = os.path.join(icdar2013_folder, 'train')
        self.gt_folder = os.path.join(icdar2013_folder, 'train_gt')
...
class Meter_Dataset(craft_base_dataset):
    def __init__(self, net, Meter_folder, target_size=300, viz=False, debug=False):
        super(Meter_Dataset, self).__init__(target_size, viz, debug)
        self.net = net
        self.net.eval()
        self.img_folder = os.path.join(Meter_folder, 'augment_image')
        self.gt_folder = os.path.join(Meter_folder, 'augment_label')
        imagenames = os.listdir(self.img_folder)
        self.images_path = []
        for imagename in imagenames:
            self.images_path.append(imagename)
...
if __name__ == '__main__':
    ...
    net.load_state_dict(
        copyStateDict(torch.load('/data2/sunjunjiao/CRAFT_Reimplementation/1-7.pth')))
    net = net.cuda()
    net = torch.nn.DataParallel(net)
    net.eval()
    dataloader = ICDAR2015(net, '/data2/sunjunjiao/CRAFT_Reimplementation/icdar2015', target_size=768, viz=True)
    train_loader = torch.utils.data.DataLoader(
        dataloader,
        batch_size=1,
        shuffle=False,
