...
if __name__ == '__main__': 
	...
	# cmp_trie.load_codec('codec_1211.txt')
	...
	with torch.no_grad():
		...
		for img_name in sorted(images):
			...
			num = int(os.path.basename(img_name).replace('ts_', "").replace("img_", "").replace('.jpg', "").replace('.JPG', "").replace('.png', ""))
				...
				if len(det_text) > 0:
					...
					for spl in splits_raw:
						...
						if len(spl[1][0]) >= eval_text_length:
							...
							res_file.write('{},{},{},{},{},{},{},{},1.0,{}\r\n'.format(int(boxw[2]), int(boxw[3]), int(boxw[4]), int(boxw[5]), int(boxw[6]), int(boxw[7]), int(boxw[0]), int(boxw[1]), spl[1][0]))		 
