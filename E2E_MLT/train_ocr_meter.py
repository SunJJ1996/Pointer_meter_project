...
def main(opts):
    ...
    for step in range(step_start, 360010):
        ...
        if step % disp_interval == 0:
            ...
            if step > step_start and (step % (batch_per_epoch / 12) == 0):
                log_f = open(log, 'a')
        ...
        if step > step_start and (step % (batch_per_epoch / 12) == 0):
            ...
            vis.line(X=torch.Tensor([step]), Y=torch.Tensor([acc_test]), win='train loss', update='append',
                     opts={'title': 'acc_test'})
            log_f = open(log, 'a')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-train_list',
                        default='/data2/sunjunjiao/data/E2Edata/train_e2e_1221/gt_e2e_train.txt')
    parser.add_argument('-valid_list',
                        default='/data2/sunjunjiao/data/E2Edata/train_e2e_1221/gt_e2e_valid.txt')
    parser.add_argument('-save_path', default='backup_1221')
    #  parser.add_argument('-model', default='/mnt/textspotter/tmp/DS_CVPR/backup2/ModelResNetSep2_25000.h5')
    parser.add_argument('-model', default='e2e-mlfdswet.h5')
